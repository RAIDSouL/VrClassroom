		...
		private void HandleCameraPreRender(Camera camera)
		{
			...
			if (sourceMaterial != null)
			{
				...
				if (sourceMaterial != null && sourceMaterial.IsKeywordEnabled("_SGT_LIGHTING") == true)
				{
					...
					if (sourceMaterial != null && sourceMaterial.HasProperty(_SGT_ScatteringStrength) == true)
					{
						scatteringStrength = sourceMaterial.GetFloat(_SGT_ScatteringStrength);
					}

					SgtLight.FilterOut(transform.position);
					SgtLight.Write(transform.position, CwHelper.UniformScale(transform.lossyScale) * OuterRadius, cachedTransform, null, scatteringStrength, SgtLight.MAX_LIGHTS);
				}
