...
Shader "Space Graphics Toolkit/Atmosphere"
{
   Properties
   {
	...
	_SGT_ScatteringMie("	Scattering Mie", Float) = 50.0
	_SGT_ScatteringRayleigh("	Scattering Rayleigh", Float) = 0.1
...
#define _USINGTEXCOORD2 1
	...
	float     _SGT_ScatteringStrength;
	float     _SGT_ScatteringMie;
	float     _SGT_ScatteringRayleigh;
			...
			float4 finalColor = main;
					...
					for (int i = 0; i < _SGT_LightCount; i++)
					{
						...
						lighting += tex2D(_SGT_LightingTex, theta) * main * _SGT_LightColor[i];
							...
							float  phase         = SGT_RayleighPhase(angle, _SGT_ScatteringRayleigh);

							scattering += tex2D(_SGT_ScatteringTex, theta) * _SGT_LightScatter[i] * phase;
						#endif
					}
			...
			float4 finalColor = main;
					...
					for (int i = 0; i < _SGT_LightCount; i++)
					{
						...
						lighting += tex2D(_SGT_LightingTex, theta) * main * _SGT_LightColor[i];
							...
							float3 worldLightDir = normalize(_SGT_LightPosition[i].xyz - wcam);
							float  angle         = dot(worldViewDir, worldLightDir);
							float  phase         = SGT_MieRayleighPhase(angle, _SGT_ScatteringMie, _SGT_ScatteringRayleigh);
