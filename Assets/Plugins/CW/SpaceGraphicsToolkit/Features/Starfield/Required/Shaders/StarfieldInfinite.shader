...
#define _USINGTEXCOORD1 1
	...
	void Ext_ModifyVertex0 (inout VertexData v, inout ExtraV2F e)
	{
			...
			float radius = v.texcoord1.x * _SGT_Scale;
				...
				float radiusMin = abs(SGT_O2V(v.vertex).z * (_SGT_ClampSizeMin / _ScreenParams.y * _SGT_ClampSizeScale));
				...
				vertexM.xyz += _SGT_StretchVector * v.texcoord1.y; // * radius;
			...
			#else
				#if _SGT_PULSE
					radius *= 1.0f + sin(v.tangent.x * 3.141592654f + _Time.x * v.tangent.y) * v.tangent.z;
				#endif
