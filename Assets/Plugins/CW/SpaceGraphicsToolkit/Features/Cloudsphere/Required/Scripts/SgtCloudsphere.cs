		...
		private void HandleCameraPreRender(Camera camera)
		{
			...
			if (sourceMaterial != null)
			{
				...
				if (material != null && material.IsKeywordEnabled("_SGT_LIGHTING") == true)
				{
					...
					var scatteringStrength = 0.0f;

					if (material.HasProperty(_SGT_ScatteringStrength) == true)
					{
						scatteringStrength = material.GetFloat(_SGT_ScatteringStrength);
					}

					SgtLight.FilterOut(transform.position);
					SgtLight.Write(transform.position, CwHelper.UniformScale(transform.lossyScale) * radius, null, null, scatteringStrength, SgtLight.MAX_LIGHTS);
				}
			}
