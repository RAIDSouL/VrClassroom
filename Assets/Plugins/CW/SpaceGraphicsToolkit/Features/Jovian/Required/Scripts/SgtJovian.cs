		...
		private void HandleCameraPreRender(Camera camera)
		{
			if (sourceMaterial != null)
			{
				...
				if (material.IsKeywordEnabled("_SGT_LIGHTING") == true)
				{
					...
					SgtLight.Write(transform.position, CwHelper.UniformScale(transform.lossyScale) * radius, transform, null, SgtLight.GetScatteringStrength(material), SgtLight.MAX_LIGHTS);
				}
