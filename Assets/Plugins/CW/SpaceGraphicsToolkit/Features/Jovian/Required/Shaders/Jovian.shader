			...
			float4 finalColor = main;
					...
					for (int i = 0; i < _SGT_LightCount; i++)
					{
						...
						lighting += tex2D(_SGT_LightingTex, theta) * main * _SGT_LightColor[i];
							...
							scattering += tex2D(_SGT_ScatteringTex, float2(theta, angle)) * _SGT_LightScatter[i];
						#endif
					}
					...
					finalColor += lighting * lerp(SGT_ShadowColor(d.texcoord2.xyz), 1.0f, finalColor.a);
