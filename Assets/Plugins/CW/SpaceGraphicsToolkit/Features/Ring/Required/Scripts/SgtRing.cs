		...
		private void HandleCameraPreRender(Camera camera)
		{
			if (sourceMaterial != null)
			{
				...
				if (material != null && material.IsKeywordEnabled("_SGT_LIGHTING") == true)
				{
					...
					SgtLight.Write(transform.position, CwHelper.UniformScale(transform.lossyScale) * radiusOuter, null, null, SgtLight.GetScatteringStrength(material), SgtLight.MAX_LIGHTS);
				}
