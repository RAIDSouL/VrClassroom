...
namespace SpaceGraphicsToolkit.LightAndShadow
{
	...
	public class SgtLight : MonoBehaviour
	{
		...
		private static int _SGT_LightScatter       = Shader.PropertyToID("_SGT_LightScatter");
		...
		public static float GetScatteringStrength(Material material)
		{
			if (material != null && material.HasProperty(_SGT_ScatteringStrength) == true)
			{
				return material.GetFloat(_SGT_ScatteringStrength);
			}

			return 0.0f;
		}

		public static void Write(Vector3 center, float radius, Transform directionTransform, Transform positionTransform, float scatterStrength, int maxLights)
		{
			var lightCount = 0;
			...
			for (var i = 0; i < tempLights.Count && lightCount < maxLights; i++)
			{
				...
				tempScatter[lightCount] = CwHelper.Brighten(color, intensity * scatterStrength);
			...
			foreach (var tempMaterial in CwHelper.tempMaterials)
			{
				...
				if (lightCount > 0)
				{
					...
					tempMaterial.SetVectorArray(_SGT_LightScatter, tempScatter);
			...
			foreach (var tempProperties in CwHelper.tempProperties)
			{
				...
				if (lightCount > 0)
				{
					...
					tempProperties.SetVectorArray(_SGT_LightScatter, tempScatter);
